...
%% infer fitness from results
% Get the values of the ith cost function for each generation
numCostFns = numCostFns;

query='Enter the number chromosomes in the population\n';
popsize=input(query);
alpha = zeros(1,numCostFns);
%% find normalise factor
for cost = 1:numCostFns
   alpha(1,cost) = array(1,cost*2+2)*popsize;
end

%% normalise putting result in an new matrix
normalised_avg  = zeros(numRows,numCostFns);
normalised_best = zeros(numRows,numCostFns);
for cost = 1:numCostFns
   normalised_avg(:,cost) = array(:,cost*2+2)/alpha(1,cost);
   normalised_best(:,cost) = array(:,cost*2+3)/alpha(1,cost);
end

%% take pythagorean sum
fitnessAvg = zeros(numRows,1);
fitnessBest = zeros(numRows,1);

%x1000 for fitness values for better visualisation
for row = 1:numRows
   % Average fitness values of the chromosome
   %fitnessAvg(row) = 1000*sqrt(sumsqr(normalised_avg(row,:)));
   sumsqr=0;
   for cols=1:numCostFns
       sumsqr=sumsqr+normalised_avg(row,cols).^2;
   end
   fitnessAvg(row) = 1000*sqrt(sumsqr);
   % Fitness value of the top chromosome
   %fitnessBest(row) = 1000*sqrt(sumsqr(normalised_best(row,:)));
   sumsqrb=0;
   for cols=1:numCostFns
       sumsqrb=sumsqrb+normalised_best(row,cols).^2;
   end
   fitnessBest(row) = 1000*sqrt(sumsqrb);
end

% Use (numCostFns+2) since we already used Figure 1 to numCostFns+1
figure(numCostFns+2);

% Plot the fitness over the number of generations evolved
plot(numGen,fitnessAvg,'b-');
hold on;
plot(numGen,fitnessBest,'r-');
%tt=strcat(y1,cf(cost),y2);
title(strcat(y1,' Fitness',y2));
xlabel('numGen');
ylabel('Fitness');
